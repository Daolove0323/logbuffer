...
public class CommentDataRepositoryImpl implements CommentDataRepository {
    ...
    @Override
    public Page<CommentResponse> findCommentsForMember(PostId postId, MemberCommentAuthorId id, Pageable pageable) {
        List<CommentResponse> res = queryFactory.
            select(new QCommentResponse(
                commentData.id.value,
                visibleContentExpr(commentData.author.memberAuthorId.eq(id)),
                commentData.state,
                new QCommentMemberResponse(
                    authorIdExpr(),
                    member.name,
    ...
    @Override
    public Page<CommentResponse> findCommentsForGuest(PostId postId, GuestCommentAuthorId id, Pageable pageable) {
        List<CommentResponse> res = queryFactory.
            select(new QCommentResponse(
                commentData.id.value,
                visibleContentExpr(commentData.author.guestAuthorId.eq(id)),
                commentData.state,
                new QCommentMemberResponse(
                    authorIdExpr(),
                    member.name,
    ...
    private Expression<String> visibleContentExpr(Predicate isMine) {
        return new CaseBuilder()
